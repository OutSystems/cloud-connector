...
* You can run `outsystemscc` on Windows or any system that supports Docker:
    * Use the `outsystemscc` Linux binary within Windows Subsystem for Linux(WSL) on Windows.
    * Use the `outsystemscc` Linux container with Docker which is fully installed within WSL2 on Windows. 
